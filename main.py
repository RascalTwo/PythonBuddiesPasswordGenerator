"""Password generator powered by tkinter collaboratively developed"""
...
class Main(tk.Tk):
    """Top-level window, containing the master style config and content frame"""
    def __init__(self):
...
class MainMenu(tk.Frame):
    """Starter menu, showing help and start options"""
    def __init__(self, master):
        ...
        Button(self, text='Exit', command=master.destroy).pack(pady=15)



class Help(tk.Frame):
    """Help screen, showng links to where the user can get help"""
    def __init__(self, master):
        tk.Frame.__init__(self, master)
        tk.Frame.config(self, bg=master['bg'])
...
class SelectMode(tk.Frame):
    """Menu to allow user to select which password type they want to generate"""
    def __init__(self, master):
        tk.Frame.__init__(self, master)
        tk.Frame.config(self, bg=master['bg'])
...
class EasyToRemember(tk.Frame):
    """Allow user to generate easy to remember password"""
    def __init__(self, master):
        tk.Frame.__init__(self, master)
        tk.Frame.config(self, bg=master['bg'])
    ...
    def generate_password(self):
        """Attempt to generate and display password to user"""
        try:
...
class FullyRandom(tk.Frame):
    """Allow user to generate fully-random passwords"""
    def __init__(self, master):
        tk.Frame.__init__(self, master)
        tk.Frame.config(self, bg=master['bg'])
